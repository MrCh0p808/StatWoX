...
export async function PATCH(
    ...
    try {
        ...
        if (survey.status === 'closed') {
            return NextResponse.json({ success: false, message: 'Survey is already closed' }, { status: 400 });
        }
        ...
        return NextResponse.json({ success: true, message: 'Survey closed successfully', data: updated });
    } catch (error) {
